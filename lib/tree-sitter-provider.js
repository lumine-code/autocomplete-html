...
// The grammars that parse HTML tags *themselves*, rather than delegating them
// to an injected `text.html.basic`. Membership is what decides which language
// layer the completions read, so it is deliberately a list of three and not a
// `text.html.` prefix test: `text.html.php`, `text.html.erb` and `text.html.ejs`
// all match such a prefix, but their tags live in an injected HTML layer, and
// matching their outer layer would hand this provider a PHP or Ruby node.
//
// tree-sitter-vue and tree-sitter-blade both embed tree-sitter-html's grammar
// wholesale, so their root layer carries `element`, `start_tag`, `tag_name` and
// `raw_text` under exactly those names. A new grammar that does the same has to
// be added here; nothing derives it, because nothing records which node types a
// grammar defines.
const HTML_TAG_SCOPES = new Set(["text.html.basic", "text.html.vue", "text.html.php.blade"]);
...
module.exports = function ({ editor, bufferPosition }) {
  ...
  const node = languageMode.getSyntaxNodeAtPosition(bufferPosition, (node, grammar) =>
    HTML_TAG_SCOPES.has(grammar.scopeName),
  );
  if (!node) return [];
